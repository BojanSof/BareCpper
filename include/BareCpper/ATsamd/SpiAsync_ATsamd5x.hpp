...
namespace BareCpper
{
  ...
  namespace SAMD51
  {
    ...
    public:
      ...
      {
        static volatile size_t iTxBuffer = 0;
        static volatile size_t iRxBuffer = 0;
    ...
    protected:
      ...
      bool initialiseClock()
      {
        ...
        const ATsamd5x::SercomClocks params = ATsamd5x::sercomClocks(sercomIndex_);
        ...
        GCLK->PCHCTRL[params.gclkId_CORE].reg = platformConfig_.gclkId_CORE_Src | GCLK_PCHCTRL_CHEN;
        GCLK->PCHCTRL[params.gclkId_SLOW].reg = platformConfig_.gclkId_SLOW_Src | GCLK_PCHCTRL_CHEN;
        // GCLK_CRITICAL_SECTION_LEAVE();

        // MCLK_CRITICAL_SECTION_ENTER();
        ATsamd5x::sercomApbEnable(sercomIndex_);
      ...
      {
        ::Sercom *sercom = ATsamd5x::sercom(sercomIndex_);
        if (!sercom)
          return false;
        hw_ = &sercom->SPI;
        ...
        const uint8_t mosiPad = *ATsamd5x::sercomPinPad(sercomIndex_, pins_.mosi); //<@note We know the result shall be valid derefernece as this is checked via sercomForPins()
        const uint8_t misoPad = *ATsamd5x::sercomPinPad(sercomIndex_, pins_.miso);
        const uint8_t sckPad = *ATsamd5x::sercomPinPad(sercomIndex_, pins_.sck);
      ...
      bool initialiseAsync()
      {
        ...
        const IRQn_Type txcIrqn = static_cast<IRQn_Type>(SERCOM0_1_IRQn + sercomIndex_ * 4);
        // RXC is at bit pos 2 in INTFLAG, so enable SERCOMn_2 IRQ
        const IRQn_Type rxcIrqn = static_cast<IRQn_Type>(SERCOM0_2_IRQn + sercomIndex_ * 4);
